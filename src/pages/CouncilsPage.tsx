...
import { fetchGalleryByCouncil } from '@/lib/data/gallery';
import type { Council, CouncilMember, DocumentItem, EventItem, GalleryItem, NewsItem, Project } from '@/lib/supabase';
  ...
  useEffect(() => {
    ...
    fetchCouncilBySlug(slug).then((found) => {
      ...
      Promise.all([
        ...
        fetchGalleryByCouncil(found.id),
        ...
        <section className="section" style={{ paddingTop: 0 }}>
          <div className="container">
            <div className="section-topline">
              <SectionHeading eyebrow="Galeri" title="Meclisten kareler." />
            ...
            <div className="gallery-grid">
              {gallery.map((item) => (
                <a href={item.media_url} target="_blank" rel="noreferrer" key={item.id}>
                  <img src={item.thumbnail_url || item.media_url} alt={item.title || 'Meclis etkinliğinden kare'} />
                </a>
              ))}
            </div>
          </div>
